...
const ListingPageComponent =props =>{
  const { enquiryModalOpenForListingId, params: previousParams } = props;
  
  const [pageClassNames, setPageClassNames] = useState([]);
  const [imageCarouselOpen, setImageCarouselOpen] = useState(false);
  const [enquiryModalOpen, setEnquiryModalOpen] = useState(enquiryModalOpenForListingId === previousParams.id);
